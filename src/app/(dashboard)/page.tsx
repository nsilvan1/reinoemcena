...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
        ...
        <section className="animate-in-view stagger-2">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-heading text-xl">Suas tarefas</h2>
              <span className="text-[10px] font-bold bg-primary text-white rounded-full h-5 min-w-5 flex items-center justify-center px-1.5">
                {data.myPendingTasks.length}
              </span>
            </div>
          </div>
            ...
            {data.myPendingTasks.map((t) => {
              ...
              return (
                <Link
                  ...
                  className={cn(
                    "block bg-card border rounded-xl p-4 hover:shadow-md transition-all group",
      ...
      {/* Pending reviews (apenas para coordenador/admin) */}
      {canReview && data.pendingReviews.length > 0 && (
        <section className="animate-in-view stagger-3">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <Eye className="h-4 w-4 text-orange-600" />
              <h2 className="font-heading text-xl">Aguardando sua revisão</h2>
              <span className="text-[10px] font-bold bg-orange-500 text-white rounded-full h-5 min-w-5 flex items-center justify-center px-1.5">
          ...
          <div className="card-elevated border rounded-xl bg-card divide-y overflow-hidden">
            {data.pendingReviews.map((r) => (
              ...
              >
                <div className="h-8 w-8 rounded-full bg-orange-100 text-orange-700 flex items-center justify-center text-[11px] font-bold shrink-0">
                  {r.editorName.charAt(0).toUpperCase()}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium">
                    <span className="text-orange-700">{r.editorName}</span> aguarda aprovação
                  </p>
        ...
        <section className="animate-in-view stagger-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-heading text-xl">Prazos próximos</h2>
            </div>
          </div>
            ...
            {data.upcomingDeadlines.map((d) => {
              ...
              return (
                <Link
                  ...
                  className="flex items-center gap-3 px-4 py-3 hover:bg-muted/30 transition-colors group border-b last:border-b-0"
                >
                  <span
                    className={cn(
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium truncate">{d.theme}</p>
                    <p className="text-[11px] text-muted-foreground truncate">
      ...
      {data.stats.totalWeeks > 0 && (
        <section className="animate-in-view stagger-5">
          ...
          <div className="card-elevated border rounded-xl bg-card p-4">
            <div className="grid grid-cols-5 gap-2">
              {PIPELINE_ORDER.map((p) => {
                ...
                return (
                  <div key={p.key} className={cn("rounded-lg p-3 text-center", p.bg)}>
                    <p className={cn("text-3xl font-bold", p.text)}>{count}</p>
                    <p className={cn("text-[10px] font-semibold uppercase tracking-wider mt-0.5", p.text)}>
                      {p.label}
                    </p>
                    <div className="h-1 bg-white/50 rounded-full mt-2 overflow-hidden">
                      <div
