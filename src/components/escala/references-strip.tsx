...
export function ReferencesStrip({
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 px-3 py-2 rounded-lg surface-1 text-xs">
        {/* História */}
              ...
              <div className="flex items-center -space-x-1.5">
                {characters.slice(0, 5).map((ch) => (
                  <button
                    ...
                    className="relative h-6 w-6 rounded-full border-2 border-background bg-muted overflow-hidden hover:z-10 hover:scale-110 transition-transform"
                    title={ch.name}
                  >
                    {ch.coverImageUrl ? (
                      /* eslint-disable-next-line @next/next/no-img-element */
                ...
                {characters.length > 5 && (
                  <span className="relative h-6 w-6 rounded-full border-2 border-background bg-muted flex items-center justify-center text-[9px] font-bold text-muted-foreground">
                    +{characters.length - 5}
                  </span>
                )}
              </div>
